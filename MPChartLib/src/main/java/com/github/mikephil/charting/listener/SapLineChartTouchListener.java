    ...
    public class SapLineChartTouchListener extends BarLineChartTouchListener {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            /** If the touch area is outside the chart return true */
           int numberOfPoints = event.getPointerCount();
           RectF chartRect = this.controllingChart.getViewPortHandler().getContentRect();

           for (int i = 0; i<numberOfPoints; i ++) {
                float xval = event.getX(i);
                float yval = event.getY(i);
               if ((yval < chartRect.top ) || (yval > chartRect.bottom) ||
                       (xval < chartRect.left) || (xval > chartRect.right) ) {
                   return true;
               }
           }
